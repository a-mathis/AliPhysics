...
Bool_t AliAnalysisTaskPLFemto::SingleParticleQualityCuts(const AliAODTrack *aodtrack,const AliAODEvent *aodEvent)
{
  ...
  if (!fIsRun1 && !aodtrack->HasPointOnITSLayer(0) &&
      !aodtrack->HasPointOnITSLayer(1) && !aodtrack->HasPointOnITSLayer(4) &&
      !aodtrack->HasPointOnITSLayer(5) && !(aodtrack->GetTOFBunchCrossing() == 0))
    return kFALSE;
...
void AliAnalysisTaskPLFemto::XiSelector(AliAODEvent *aodEvent)
{
  ...
  for (Int_t i=0; i<entriesXi; i++)
      ...
      else if(Xi->ChargeXi() < 0)//Xim
	{
	  ...
	  AliAODTrack* pTrack = fGTI[Xi->GetPosID()];//proton (Lambda)
	  AliAODTrack* nTrack1 = fGTI[Xi->GetBachID()];//pion(-)
	  AliAODTrack* nTrack2 = fGTI[Xi->GetNegID()];//pion(-) (Lambda)
...
void AliAnalysisTaskPLFemto::PairAnalysis(const AliFemtoProtonParticle &protonA,const AliFemtoProtonParticle &protonB,Int_t REorME,pairType inputPair)
{
  ...
  if(REorME == 0 && inputPair == kProtonProton)//real event
  ...
  else if(REorME!=0 && inputPair == kProtonProton)//mixed event
    {
      ...
      fillthis = "fProtonProtonRelKMEMulti";
      fillthis += MultBin;
      ((TH1F*)(fOutputTP->FindObject(fillthis)))->Fill(relK);
...
void AliAnalysisTaskPLFemto::DefineHistograms(TString whichV0)
{
  ...
  TH1F* fProtonLambdaRelKMulti[12];
  TH1F* fProtonLambdaRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fAntiProtonAntiLambdaRelKMulti[12];
  TH1F* fAntiProtonAntiLambdaRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fProtonXiRelKMulti[12];
  TH1F* fProtonXiRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fAntiProtonXiRelKMulti[12];
  TH1F* fAntiProtonXiRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fProtonProtonRelKMulti[12];
  TH1F* fProtonProtonRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fAntiProtonAntiProtonRelKMulti[12];
  TH1F* fAntiProtonAntiProtonRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fLambdaLambdaRelKMulti[12];
  TH1F* fLambdaLambdaRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
  ...
  TH1F* fAntiLambdaAntiLambdaRelKMulti[12];
  TH1F* fAntiLambdaAntiLambdaRelKMEMulti[12];
  for(int i=0; i<12; ++i) {
